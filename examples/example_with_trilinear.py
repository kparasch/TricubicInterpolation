...
il.set_steps(dx, dy, dz)
il.set_origin(x0, y0, z0)
il.initialize(A)
...
X = np.linspace(x0+(1+discard_x)*dx,x0+(Nx-3-discard_x)*dx,3000)
Y = np.linspace(y0+(1+discard_y)*dy,y0+(Ny-3-discard_y)*dy,3000)
Z = np.linspace(z0+(1+discard_z)*dz,z0+(Nz-3-discard_z)*dz,3000)
...
ax1.plot(X, np.array([il.val(i,y_obs,z_obs) for i in X]),'b')
ax1.set_xlabel('x')
ax1.set_title('f')
...
ax2.plot(X, np.array([il.ddx(i,y_obs,z_obs) for i in X]),'b')
ax2.set_xlabel('x')
ax2.set_title('df/dx')
...
ax3.plot(X, np.array([il.ddy(i,y_obs,z_obs) for i in X]),'b')
ax3.set_xlabel('x')
ax3.set_title('df/dy')
...
ax4.plot(X, np.array([il.ddz(i,y_obs,z_obs) for i in X]),'b')
ax4.set_xlabel('x')
ax4.set_title('df/dz')
...
ax7.plot(Y, np.array([il.ddy(x_obs,i,z_obs) for i in Y]),'b')
...
ax9.plot(Z, np.array([il.val(x_obs,y_obs, i) for i in Z]),'b')
ax9.set_xlabel('z')

ax10=fig.add_subplot(3,4,10)
ax10.plot(Z, lamdfdx(x_obs, y_obs, Z),'k')
ax10.plot(Z, np.array([ip.ddx(x_obs,y_obs, i) for i in Z]),'r')
ax10.plot(Z, np.array([il.ddx(x_obs,y_obs, i) for i in Z]),'b')
ax10.set_xlabel('z')
