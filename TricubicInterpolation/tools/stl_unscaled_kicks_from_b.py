...
SIXTRL_STATIC SIXTRL_FN void NS(tricub_unscaled_kicks_from_b)(
...
SIXTRL_INLINE void NS(tricub_unscaled_kicks_from_b)(
    SIXTRL_ARGPTR_DEC NS(be_tricub_real_t) const* SIXTRL_RESTRICT b,
    NS(be_tricub_real_t) x1,
    NS(be_tricub_real_t) y1,
    NS(be_tricub_real_t) z1,
...
//    NS(be_tricub_real_t) const tri_consts[9] = {2.0, 3.0, 4.0, 6.0, 8.0, 9.0, 12.0, 18.0, 27.0};
    ...
    NS(be_tricub_real_t) x2 = x1*x1;
    NS(be_tricub_real_t) y2 = y1*y1;
    NS(be_tricub_real_t) z2 = z1*z1;

    NS(be_tricub_real_t) x3 = x2*x1;
    NS(be_tricub_real_t) y3 = y2*y1;
    NS(be_tricub_real_t) z3 = z2*z1;

''')

tc = ['2.0', '3.0', '4.0',  '6.0', '8.0', '9.0', '12.0', '18.0', '27.0']
def write_coef(ff,i,j,k,sp_list):
    ii = i + 4 * j + 16 * k
    flag = 1
    for jj in range(64):
